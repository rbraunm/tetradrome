...
"""The host-platform axis: everything OS-specific the scheduler needs, one class per host.

``HostPlatform`` owns discovering the NUMA topology, the true memory ceiling, pinning a worker
to cores, reading a process's private memory, and whether fresh workers share their imports.
``for_host()`` returns the implementation for the OS we are on, raising loudly on an
unrecognized host so that supporting it is a deliberate addition, not a silent guess. Linux
reads /sys and the cgroup; Windows uses ctypes against kernel32/psapi. The fiddly /sys parsing
is split into pure functions so the formats are tested directly.

Everything above this layer is platform-agnostic and consumes the resulting NumaNode list and
ceiling via the Machine that ``inventory.detect_machine`` assembles.
"""
...
from .inventory import NumaNode
