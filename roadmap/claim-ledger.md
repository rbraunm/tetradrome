...
| Acceleration tiers agree with the reference | Yellow | every tier — bit-packed (numpy/CPU + cupy/GPU), numba JIT, dense GPU kernel, multi-core + NUMA pinning, size/VRAM routing, multimodular ℚ — reproduces the reference homology across the catalog; the GPU and numba paths are validated here through their un-accelerated shared code | code-complete; the GPU/numba/multi-socket *speedups* await validation on real hardware (run the suite + `bench_reducers.py` there), then calibrate the routing threshold |
