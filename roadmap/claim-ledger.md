...
| Acceleration tiers agree with the reference | Yellow | bit-packed F2 tier (numpy/CPU + cupy/GPU from one body of code), runtime registry with loud fallback, GPU detection/auto-config/guidance, multi-core parallel reduction, size/VRAM-aware routing, and the multimodular ℚ path — all reproduce the reference homology across the catalog | Phase 5 in progress — Numba JIT, a batched GPU kernel, and NUMA pinning remain (best calibrated on real hardware) |
